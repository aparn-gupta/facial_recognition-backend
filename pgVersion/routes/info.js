...
infoRouter.get("/test", async (req, res) => {
...
infoRouter.post("/newfeeling/:userid", authMiddleware, async (req, res) => {
  if (!req.body.title && !req.body.mood && !req.body.description) {
    res.status(400).json({
      success: false,
      message:
        "Can't process empty request body. Please fill atleast one of the three fields.",
    });
    return;
  }
  ...
  try {
    const result = await pool.query(
      "INSERT INTO feelings (title, mood, feeling_notes, post_time, user_id) VALUES ($1, $2, $3, $4, $5);",
      [title, mood, description, currentTime, userId],
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Posting failed, server error" });
  }
});
...
infoRouter.get("/allfeelings/:userid", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * from feelings where user_id  = $1 order by feeling_id",
      [userId],
    );
