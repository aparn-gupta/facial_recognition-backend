...
authRouter.post("/registerface", async (req, res) => {
  ...
  const result = await pool.query("SELECT * FROM facedata");
  ...
  if (!matchResult) {
    ...
    try {
      const result = await pool.query(
        "INSERT INTO facedata (username,  descriptorArr) VALUES ($1, $2)",
...
try {
  ...
  const resultRows = await pool.query("INSERT INTO facedata (username, password) VALUES ($1, $2)", [manualUsername, hashedPassword])
...
try {
  ...
  const [rows] = await pool.query("SELECT * FROM facedata WHERE username = $1", [manualUsername])
